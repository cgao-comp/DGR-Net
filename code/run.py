...
from trains.singleTask.model.DGR import DGR
...
def DGR_run(
    ...
    seeds = seeds if seeds != [] else [1111, 1112, 1113, 1114, 1115]##随机种子定义，这是为了消除偶然性，确保模型性能的稳定性
...
def _run(args, num_workers=4, is_tune=False, from_sena=False): 
    ...
    if args.is_training:
        ...
        model_DGR = model_DGR.cuda()

        model = [model_DGR]         
    else:
        print("testing for DGR")
        model_DGR = DGR(args)

        model = model_DGR.cuda()
